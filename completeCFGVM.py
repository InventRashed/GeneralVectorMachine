# -*- coding: utf-8 -*-
"""
Created on Mon Nov  1 14:05:41 2021
...
def compute_loss(d, y_pred, y_true, C):
    """
...
#%%
def Fitness(listM, listP, listG, cols, X_train, y_train, X_test, y_test, params, method="feature"):
    #fitness function
    alpha=0.01
    d = params[0]
    input_dim = params[1]
    ...
    if np.sum(cols)==0:
        ...
        if method=="feature":
            ...
            model_train.save(r"C:\Users\rmuslem\Capgemini\Stage Rashed - General\models"+'\\'+'modelF'+str(len(listP)))
...
def bALO(N,Max_iter,X_train, y_train, X_test, y_test, GVMparams, method='feature'):
    ...
    for i in range(N):
        antlions_fitness[i], metricsList, posList, gmeanList=Fitness(metricsList,  posList, gmeanList, antlion_position[i], X_train, y_train, X_test, y_test, GVMparams, method )
    ...
    #         
            ants_fitness[i], metricsList, posList,gmeanList = Fitness(metricsList, posList, gmeanList, ant_position[i], X_train, y_train, X_test, y_test, GVMparams, method )  
...
def CFGVM(N_C, N_F, Max_iter_C, Max_iter_F, X_train, y_train, X_test, y_test, X_all, y_all, paramsC):
    ...
    fitnessC, positionC, metricsC, posC, GC = bALO(N_C, Max_iter_C, X_train, y_train, X_test, y_test, paramsC, 'cost')
    ...
    fitness, position, metricsF, posF, GF= bALO(N_F, Max_iter_F, X_train, y_train, X_test, y_test, paramsC, 'feature')
