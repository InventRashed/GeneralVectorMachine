...
#%%
def Fitness(listM, listP, listG, cols, X_train, y_train, X_test, y_test, params, method="feature",save='CFGVM'):
    #fitness function
    alpha=0.01
    d = params[0]
    input_dim = params[1]
    ...
    if np.sum(cols)==0:
        ...
        if method=="feature":
            ...
            X_tr = X_train[:, x]
            X_t = X_test[:, x]
            model_train = train(X_tr, y_train, d, input_dim, b, batch_size, C, lr, epochs, nodes, ranB)
            y_pred = model_train(X_t)

            model_train.save(r"C:\Users\rmuslem\Capgemini\Stage Rashed - General\models"+'\\'+save+'\\'+'modelF'+str(len(listP)))

            metrics = MetricsClass(y_test, y_pred) 
            listG.append(metrics['Gmean'])
            listM.append(metrics)
            y=(1-alpha)*(1-metrics['Gmean'])+alpha*np.sum(x)/(len(x))
        elif method=="cost":
            ...
            listP.append(cols)
            model_train.save(r"C:\Users\rmuslem\Capgemini\Stage Rashed - General\models"+'\\'+save+'\\'+'modelC'+str(len(listP)))
...
def bALO(N,Max_iter,X_train, y_train, X_test, y_test, GVMparams, method='feature', Save='CFGVM'):
    """
    ...
    for i in range(N):
        antlions_fitness[i], metricsList, posList, gmeanList=Fitness(metricsList,  posList, gmeanList, antlion_position[i], X_train, y_train, X_test, y_test, GVMparams, method, Save )
    ...
    #         
            ants_fitness[i], metricsList, posList, gmeanList = Fitness(metricsList, posList,gmeanList, ant_position[i], X_train, y_train, X_test, y_test, GVMparams, method, Save )  
    ...
    # fprintf('\n');
    return Elite_antlion_fitness,Elite_antlion_position, metricsList, posList, gmeanList
#%%
# def Fitness(cols, X_train, y_train, X_test, y_test, params, listW, listM, listF, listB, method="feature"):
#     #fitness function
#     alpha=0.01
#     d = params[0]
#     input_dim = params[1]
...
#             X_tr = X_train[:, x]
#             X_t = X_test[:, x]
#             model, beta1 = CGVM(X_tr, y_train, d, input_dim, b, batch_size, C, lr, epochs, nodes)
#             y_pred = model(X_t)
#             listW.append(model.get_weights())
#             weightss_E = model.get_weights()
#             metrics = MetricsClass(y_test, y_pred) 
#             listM.append(metrics['Gmean'].numpy())
#             listB.append(beta1)
#             y=(1-alpha)*(1-metrics['Gmean'])+alpha*np.sum(x)/(len(x))
#         elif method=="cost":
#             C1 = int("".join(str(x) for x in cols[0:4]),2)
...
#             if C<1:
#                 C=C+1
#             model, beta1 = CGVM(X_train, y_train, d, input_dim, b, batch_size, C, lr, epochs, nodes)
#             listF.append(C)
#             y_pred = model(X_test)
#             listW.append(model.get_weights())
#             weightss_E = model.get_weights()
...
#             listM.append(metrics['Gmean'].numpy())
#             listB.append(beta1)
#             y = (1-metrics['Gmean']).numpy()
#         else:
#             raise ValueError('Incorrect input for method, possible values: feature or cost')
        
#     return y, beta1, weightss_E, listW, listM, listF, listB
...
# def bALO(N,Max_iter,X_train, y_train, X_test, y_test, GVMparams, method='feature'):
#     """
...
#     # Calculate the fitness of initial antlions and sort them
#     for i in range(N):
#         antlions_fitness[i], beta_antlions[i], weight_antlions[i], weightList, metricsList, featuresList, betaList=Fitness(antlion_position[i], X_train, y_train, X_test, y_test, GVMparams, weightList, metricsList, featuresList, betaList, method )
...
#     Elite_antlion_position=Sorted_antlions[0]
#     Elite_antlion_fitness=sorted_antlion_fitness[0]
#     elite_beta = beta_antlions[sorted_indexes[0]]
#     elite_weight = weight_antlions[sorted_indexes[0]]
...
#             # RA is the random walk around the selected antlion by rolette wheel
#             #RA=RandWalk(dim,Max_iter,lb,ub, Sorted_antlions(Rolette_index,:),Current_iter);
#             RA=MutationU(dim,Max_iter,Sorted_antlions[Rolette_index,:],Current_iter)
#             # RE is the random walk around the elite (best antlion so far)
#            # [RE]=RandWalk(dim,Max_iter,Elite_antlion_position(1,:),Current_iter);
#             RE=MutationU(dim,Max_iter,Elite_antlion_position,Current_iter)
#             #ant_position(i,:)= (RA(Current_iter,:)+RE(Current_iter,:))/2; # Equation (2.13) in the paper   
#             ant_position[i,:]= CrossOverU(RA,RE)
...
#     #         ant_position(i,:)=(ant_position(i,:).*(~(Flag4ub+Flag4lb)))+ub.*Flag4ub+lb.*Flag4lb;  
#     #         
#             ants_fitness[i], beta_ants[i],  weight_ants[i], weightList, metricsList, featuresList, betaList=Fitness(ant_position[i], X_train, y_train, X_test, y_test, GVMparams, weightList, metricsList,featuresList, betaList, method )  
#             #All_fitness[0,i]=ants_fitness[0,i]
...
#         double_population=np.concatenate((Sorted_antlions, ant_position))
#         double_fitness=np.concatenate((sorted_antlion_fitness, ants_fitness))
#         double_beta = np.concatenate((beta_antlions, beta_ants))
#         double_weight = weight_antlions + weight_ants
...
#             elite_beta = double_beta[I[0]]
#             elite_weight = double_weight[I[0]]
#     #         fprintf('\t');
        
#         me=np.mean(antlions_fitness)
#         den=np.max(abs(antlions_fitness-me))
#         if den<1:
#             den=1
        
#         sig=np.sum(np.square((antlions_fitness-me)/den))/N
#         print('bALO: ',Elite_antlion_fitness)
#         print(Elite_antlion_position)
#         print('\n')
#         print(sig)
...
#         beta_antlions[0] = elite_beta
#         weight_antlions[0]=elite_weight
...
#     # fprintf('\n');
#     return Elite_antlion_fitness,Elite_antlion_position, elite_beta, elite_weight, weightList, metricsList, featuresList, betaList
